 ...
 */
public class CommentListAdapter extends RecyclerView.Adapter<CommentListAdapter.ImageViewHolder> {
    ...
    public CommentListAdapter(List<GroupCommentItem> list, OnItemClickListener listener, String type, String groupId, String postId, String teamId, int count) {
        if (list == null) return;
        this.list = list;
...
//        holder.txtName.setText(item.fullname);
        ...
        if (item.isLiked)
        {
            holder.iv_Like.setImageResource(R.drawable.icon_post_liked);
            //holder.txt_like.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_thumb_up_active, 0, 0, 0);
        }
        else
        {
                holder.iv_Like.setImageResource(R.drawable.icon_post_like);
        ...
        if (!TextUtils.isEmpty(item.createdByImage)) {
            holder.img_profile_default.setVisibility(View.INVISIBLE);
            Picasso.with(mContext).load(Constants.decodeUrlToBase64(item.createdByImage)).resize(dpToPx(), dpToPx()).networkPolicy(NetworkPolicy.OFFLINE).into(holder.img_profile,
                    new Callback() {
                        @Override
                        ...
                        public void onError() {
                            ...
                            Picasso.with(mContext).load(Constants.decodeUrlToBase64(item.createdByImage)).fit().into(holder.img_profile, new Callback() {
                                @Override
                                public void onSuccess()
                                {
                                ...
                                public void onError()
                                {
                                    ...
                                    TextDrawable drawable = TextDrawable.builder()
                                            .buildRound(ImageUtil.getTextLetter(item.createdByName), ImageUtil.getRandomColor(position));//, mContext.getResources().getDimensionPixelSize(R.dimen.padding_20dp));
...
//        mAdapter = new CommentListAdapter(commentList, this, "group", groupId, postId);
    ...
    public class ImageViewHolder extends RecyclerView.ViewHolder {
        ...
        @Bind(R.id.txt_like_list)
        TextView txt_like_list;
        ...
        @OnClick({R.id.txt_like, R.id.iv_like , R.id.txt_reply, R.id.txt_edit, R.id.txt_name,R.id.txt_like_list})
        public void OnLikeClick(View v) {
            ...
            switch (v.getId()) {
                ...
                case R.id.txt_edit:
                    if (isConnectionAvailable()) {
                        listener.onEditClick(item, getLayoutPosition());
                    } else {
                        showNoNetworkMsg();
                    }
                    break;
                ...
                case R.id.txt_reply:
                    if (isConnectionAvailable()) {
                        ...
                        intent.putExtra("comment_id", list.get(getLayoutPosition()).id);
                        intent.putExtra("user_id", CommentsFragment.selectedUserId);
    ...
    public interface OnItemClickListener {
        ...
        void onNameClick(GroupCommentItem item);

        void onLikeListClick(GroupCommentItem item);
    }
