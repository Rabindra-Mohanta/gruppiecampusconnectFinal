...
public class TestExamEditActivity extends BaseActivity implements OnPhotoEditorListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        assignmentData = new Gson().fromJson(getIntent().getStringExtra("item"), TestPaperRes.TestPaperData.class);
        ...
        mPhotoEditor.saveAsFile(ImageUtil.getOutputMediaFile().getAbsolutePath() , saveSettings, new PhotoEditor.OnSaveListener() {
            ...
            public void onSuccess(@NonNull String imagePath)
            {
                ...
                if(!editedPaths.get(currentPage).equalsIgnoreCase("xxx"))
                    Picasso.with(TestExamEditActivity.this).load(new File(editedPaths.get(currentPage))).into(ivImage.getSource());
                else
                Picasso.with(TestExamEditActivity.this).load(Constants.decodeUrlToBase64(assignmentData.fileName.get(currentPage))).into(ivImage.getSource());
...
//loading font from asset
    ...
    private void showAddTextDialog() {
        ...
        ivTrophy.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mPhotoEditor.addImage(getBitmapFromVectorDrawable(TestExamEditActivity.this, R.drawable.icon_trophy));
        ...
        ivCrown.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mPhotoEditor.addImage(getBitmapFromVectorDrawable(TestExamEditActivity.this, R.drawable.icon_crown));
                dialog.dismiss();
            }
        });
    ...
    private void notVerifyAssignment() {
        ...
        dialog.findViewById(R.id.btnAdd).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                    if (ActivityCompat.checkSelfPermission(TestExamEditActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                ...
                mPhotoEditor.saveAsFile(ImageUtil.getOutputMediaFile().getAbsolutePath() , saveSettings, new PhotoEditor.OnSaveListener() {
                        ...
                        public void onSuccess(@NonNull String imagePath)
                        {
                            ...
                            progressDialog = new ProgressDialog(TestExamEditActivity.this);
                            progressDialog.setMessage("Uploading Image...");
                            progressDialog.show();

                            editedPaths.set(currentPage , imagePath);

                            comments = etTitle.getText().toString().trim();
                           // uploadImageOnCloud(imagePath,etTitle.getText().toString().trim());
                            uploadThumbnail(editedPaths,0,true);
                        }
    ...
    private void uploadThumbnail(ArrayList<String> listThumbnails, int index, boolean checked) {
        if (index == listThumbnails.size())
        {
            progressDialog.dismiss();
            Intent intent = new Intent();
            intent.putExtra("isVerify", checked);
            ...
            try {
                ...
                observer.setTransferListener(new TransferListener() {
                    ...
                    public void onError(int id, Exception ex) {
                        ...
                        Toast.makeText(TestExamEditActivity.this, getResources().getString(R.string.image_upload_error), Toast.LENGTH_SHORT).show();
                    }
                });
            } catch (IOException e) {
                e.printStackTrace();
